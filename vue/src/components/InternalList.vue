<template>
  <div>
    <div class="card" style="min-height: 80vh">
      <div class="inter-box" v-for="item in tableData" :key="item.id" v-if="total > 0">
        <div style="flex: 1; width: 0">
          <a :href="'/front/interDetail?interId=' + item.id" target="_blank"><div class="inter-title">{{ item.title }}</div></a>
          ...
          <div style="display: flex; align-items: center">
            <div style="flex: 1; font-size: 13px">
              ...
              <span v-if="showOpt" style="margin-left: 10px; color: #2a60c9; cursor: pointer" @click="editinter(item.id)"><i class="el-icon-edit"></i>编辑</span>
            </div>
...
export default {
  ...
  methods: {
    editinter(interId) {
      window.open('/front/newinter?interId=' + interId)
    },
    del(id) {   // 单个删除
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/inter/delete/' + id).then(res => {
    ...
    loadinters(pageNum) {
      ...
      let url
      switch (this.type) {
        case 'user': url = '/internalReferral/selectUser'; break;
        case 'like': url = '/internalReferral/selectLike'; break;
        case 'collect': url = '/internalReferral/selectCollect'; break;
        case 'comment': url = '/internalReferral/selectComment'; break;
        default: url = '/internalReferral/selectPage'
      }
      this.$request.get(url, {
        params: {
          pageNum: this.pageNum,
          pageSize: this.pageSize,
          categoryName: this.categoryName === '全部内推' ? null : this.categoryName,
